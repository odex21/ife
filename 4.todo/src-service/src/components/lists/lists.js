...
export default class List extends Component {
    ...
    attached() {
        ...
        const str = state
        const isAll = ((str !== "" && str !== null) ? false : true)
        this.data.set('isAll', isAll)
        // const t = this.data.get('isAll')
        this.actions.lists(str)//更新todos
        const that = this
    ...
    static computed = {
        ...
        isAll: function () {
            const str = this.data.get('selected')
            console.log(str + "_______________")
    ...
    complete(l, i) {
        ...
        const isRM = !this.data.get('isAll')
        if (isRM) this.data.set("bArr." + i, '')
        console.log(isRM)
        setTimeout(() => {
            this.actions.done({ list: l, isRM: isRM })
        }, 0)
    }
    ...
    transition() {
        if (arguments[3] === 0 || arguments[2].length < 15) {

            return transition(arguments[0], arguments[1])
        }
    }
